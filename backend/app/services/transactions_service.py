...
async def get_transactions(current_user_id: int, role: str) -> list[dict]:
  try:
    pool = await get_pool()
...
async def get_transaction_by_id(tx_id: int, current_user_id: int, role: str) -> dict | None:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      if role == "admin":
    ...
    logger.exception(f"Error fetching transaction id: {tx_id}")
    raise


async def get_transaction_history(current_user_id: int, role: str) -> list[dict]:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      if role == "admin":
        rows = await conn.fetch(
          ...
          SELECT
            ...
            lh.action,
            lh.action_taken_at,
            lh.old_data->>'amount' AS old_amount,
            lh.new_data->>'amount' AS new_amount,
          ...
          FROM log_history lh
          LEFT JOIN transactions t ON lh.entity_id = t.id
      ...
      return [helpers.format_action_taken_at(dict(row)) for row in rows]
  except Exception:
    logger.exception("Error fetching transaction history")
    raise


async def create_transaction(tx, current_user_id: int) -> dict | None:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
...
async def update_transaction(tx_id: int, tx, current_user_id: int, role: str) -> dict | None:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      async with conn.transaction():
        ...
        await conn.execute(
          """
          INSERT INTO log_history (entity_type, entity_id, user_id, action, old_data, new_data, action_taken_at)
...
async def delete_transaction(tx_id: int, current_user_id: int, role: str) -> bool | None:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      async with conn.transaction():
...
async def create_transaction_deletion_request(tx_id: int, requested_by: int) -> dict | None:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      existing = await conn.fetchrow(
        "SELECT * FROM transaction_deletion_requests WHERE transaction_id = $1 AND status = 'pending'",
        tx_id,
      )
      if existing:
        return None
    ...
    # Fire-and-forget outside connection block so notification failure
    # never rolls back the successfully inserted request.
...
async def get_deletion_requests() -> list[dict]:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      rows = await conn.fetch(
...
async def review_deletion_request(request_id: int, admin_id: int, approve: bool) -> dict | None:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      async with conn.transaction():
        req = await conn.fetchrow(
          "SELECT * FROM transaction_deletion_requests WHERE id = $1",
          request_id,
        )
    ...
    # Fire-and-forget outside transaction block so notification failure
...
async def get_deletion_requests_my_history(current_user_id: int, role: str) -> list[dict]:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      if role == "admin":
      ...
      for row in rows:
        ...
        data["reviewer"] = (
          ...
          if rev_first is not None else None
        )
        if data.get("amount") is not None:
          data["transaction"] = {
            "id": data["transaction_id"],
          ...
          for col in ("amount", "transaction_type", "category_id", "description", "transaction_date", "category_name"):
            data.pop(col, None)
          data["transaction"] = None
        result.append(data)
      return result
  except Exception:
    logger.exception(f"Error fetching deletion request history for user {current_user_id}")
    raise


async def cancel_deletion_request(request_id: int, user_id: int) -> bool | None:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      req = await conn.fetchrow(
      ...
      if not req or req["requested_by"] != user_id or req["status"] != "pending":
        return None
      await conn.execute(
        "DELETE FROM transaction_deletion_requests WHERE id = $1",
        request_id,
      )
      return True
  except Exception:
    logger.exception(f"Error cancelling deletion request {request_id}")
    raise


async def count_transactions_by_category(category_id: int, current_user_id: int, role: str) -> int:
  try:
    pool = await get_pool()
    async with pool.acquire() as conn:
      if role == "admin":
        ...
        count = await conn.fetchval(
          "SELECT COUNT(*) FROM transactions WHERE category_id = $1 AND user_id = $2 AND deleted_at IS NULL",
          category_id,
          current_user_id,
        )
      return count or 0
